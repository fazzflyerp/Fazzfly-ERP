...
export async function GET(request: NextRequest) {
  try {
    ...
    if (availableOnly) {
      ...
      const stockRows = await saReadRange(targetSheetId, "INV_Stock!A:N", fresh ? 0 : undefined);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Read INV_Stock — FIFO: sort by expiry_date ASC (A:N includes cost_per_unit at index 13)
