/**
 * General Form Submit API - CONCURRENT SAFE ✅
 * POST /api/module/submit-general
 * 
 * ✅ Optimistic locking (prevent race condition)
...
// ✅ Parse date - รองรับ DD/MM/YYYY และ YYYY-MM-DD
...
// ✅ สร้าง snapshot checksum สำหรับตรวจสอบการเปลี่ยนแปลง
function createSnapshot(rows: any[][]): { checksum: string; rowCount: number } {
  // เอาแค่ 5 แถวล่างสุด + rowCount
...
// ✅ Main insert logic with retry mechanism
async function attemptInsertWithRetry(params: {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(
        `📊 [${requestId}] Snapshot: ${snapshot.rowCount} rows, checksum: ${snapshot.checksum}`
      );

      // 2️⃣ คำนวณตำแหน่ง insert (DATE SORT)
      let insertIndex = snapshot.rowCount; // default: append ท้ายสุด
      ...
      if (dateFieldIndex !== null && rowsToInsert.length > 0) {
        ...
        if (newDate) {
          ...
          if (insertIndex === 1) {
            console.log(
              `📍 [${requestId}] Insert at row ${insertIndex} (oldest date)`
            );
          }
        } else {
          console.warn(
            `⚠️ [${requestId}] Cannot parse date: "${newDateStr}", appending to end`
          );
          insertIndex = snapshot.rowCount;
        }
      } else {
        console.log(`\n⚠️ [${requestId}] No date field, appending to end`);
      }
      ...
      if (currentSnapshot.checksum !== snapshot.checksum) {
        console.warn(
          `⚠️ [${requestId}] Sheet modified during preparation!`
        );
        console.warn(
          `   Expected: ${snapshot.checksum}, Got: ${currentSnapshot.checksum}`
        );
      ...
      console.log(
        `⏳ [${requestId}] Inserting ${rowsToInsert.length} rows at ${insertIndex}...`
      );
      ...
      const writeRange = `${sheetName}!A${insertIndex}:${endCol}${
        insertIndex + rowsToInsert.length - 1
      }`;
      ...
      console.log(
        `✅ [${requestId}] SUCCESS! Inserted at row ${insertIndex}`
      );
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`📋 [${requestId}] GENERAL FORM SUBMIT API (CONCURRENT SAFE)`);
    console.log("=".repeat(80));
    ...
    for (const field of fields) {
      ...
      if (order !== undefined && order !== null && order !== "") {
        ...
        if (colIndex >= 0 && colIndex < headerRow.length) {
          ...
          console.log(
            `   ✅ "${field.fieldName}" → Column ${colIndex + 1} (${getColumnLetter(
              colIndex + 1
            )})`
          );
          ...
          if (field.type === "date" && dateFieldIndex === null) {
            ...
            console.log(`   📅 Date field found at column ${colIndex + 1}`);
          }
        }
      }
    }
    ...
    console.log(`\n📊 [${requestId}] Mapped: ${mappedCount} fields`);

    if (mappedCount === 0) {
      return NextResponse.json(
        { error: "No fields mapped", code: "NO_MAPPING" },
        { status: 400 }
      );
    }
    ...
    lineItems.forEach((item: any, idx: number) => {
      ...
      console.log(`   Row ${idx + 1}: ${filledCount} cells filled`);
    });
