...
import { ConfigField, getPeriodOptions } from "./salesUtils";
      ...
      {(hasDateFilter || selectedPeriods.length > 0 || selectedYear) && (
        <div className="mt-4 lg:mt-5 flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
          {(hasDateFilter || selectedPeriods.length > 0) && (
            <button
              onClick={() => { selectedPeriods.forEach((p) => onPeriodToggle(p)); onDateFromChange(""); onDateToChange(""); }}
              disabled={loading}
          ...
          {selectedYear && (
            <button
              onClick={() => { onYearChange(null); selectedPeriods.forEach((p) => onPeriodToggle(p)); onDateFromChange(""); onDateToChange(""); onClearFilters(); }}
              disabled={loading}
