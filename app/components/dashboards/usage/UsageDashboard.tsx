/**
 * Usage Dashboard
 * Location: app/components/dashboards/usage/UsageDashboard.tsx
 * ✅ Complete Usage Dashboard with filters, KPI, charts, ranking
 * ✅ FIXED: API calls now match Sales Dashboard 100%
 */
...
export default function UsageDashboard({
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // ✅ Step 1: Determine target spreadsheet
      let targetSpreadsheetId = spreadsheetId;

      if (selectedYear && archiveFolderId) {
    ...
    return (
      <div className="flex items-center justify-center p-12">
        <div className="text-center">
          ...
          <p className="text-slate-600">กำลังโหลดข้อมูล Usage Dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Ranking */}
      <UsageRanking rankingTableData={rankingTableData} />
    </div>
  );
}