      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {isCompare ? (
            ...
            <>
              ...
              <button
                onClick={() => { setMode("compare"); setActivePeriod(""); }}
                className="px-3 py-1.5 rounded-xl text-xs font-medium bg-white/5 border border-white/10 text-slate-400 hover:text-white hover:bg-white/10 transition-all"
              >
                เปรียบเทียบ 2 ช่วง
              </button>
        ...
        {isDod && (
          <div className="flex flex-wrap items-center gap-2">
            ...
            <input
              type="date" value={dodStart} onChange={(e) => setDodStart(e.target.value)}
            ...
            <span className="text-slate-600 text-xs">–</span>
            <input
              type="date" value={dodEnd} onChange={(e) => setDodEnd(e.target.value)}
              className="px-2 py-1.5 rounded-xl bg-white/5 border border-white/10 text-slate-300 text-xs focus:outline-none focus:border-emerald-500/50"
            />
            {(dodStart || dodEnd) && (
              <button
                onClick={() => { setDodStart(""); setDodEnd(""); }}
            ...
            <button
              onClick={() => setDodSort((s) => s === "asc" ? "desc" : "asc")}
